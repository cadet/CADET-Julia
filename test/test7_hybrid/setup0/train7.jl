...
cin = 5.5
q_scale = 55.54*1.8*cin^1.0/(1.0 + 1.8*cin^1.0)
	
# Solving w. Initial weights 
# p vector should include p_init from lux.componentarray
rhs_test = hybrid_model(columns, columns[1].RHS_q, columns[1].cpp, columns[1].qq, 1, solverOptions.nColumns, solverOptions.idx_units, switches, q_scale)
...
plot!(c_exp_data[1:end, 1], train_prediction[1:end]*cin, label = "neural UDE", legend=:bottomright)
display(fig)
