...
# Tests spatial convergence using a sharp rectangular pulse injection
# More challenging test case with discontinuous inlet BC
...
println("=" ^ 80)
println("EOC TEST - RECTANGULAR PULSE INJECTION")
...
function inlet_rectangular(t)
    """Sharp rectangular pulse inlet boundary condition."""
...
println("  Rectangular pulse: c = $c_inject for t ∈ [$t_inject_start, $t_inject_end] s")
println()
...
println("  Note: Rectangular pulse has discontinuous BC - may show reduced convergence")
println()
...
for polyDeg in polyDeg_list
    ...
    # ==================== SAVE RESULTS ====================
    output_file = "test12_EOC_rectangular_pulse_p$(polyDeg).csv"
    open(output_file, "w") do io
        println(io, "# EOC Test - Rectangular Pulse Injection in Radial Flow")
        println(io, "# rin=$rin m, rout=$rout m, v=$v m/s, D=$D m²/s")
        println(io, "# polyDeg=$polyDeg (fixed), t_final=$t_final s")
        println(io, "# Rectangular pulse: c=$c_inject for t in [$t_inject_start, $t_inject_end] s")
    ...
    println("  - Smooth solutions: p+1 = $(polyDeg+1)")
    println("  - Discontinuous BC: May show reduced rate due to sharp fronts")
    println("  - Acceptable for rectangular pulse: ≥ 1.0 (first order)")
    println()

    # Check if we achieved acceptable convergence
    if length(errors_L2) >= 2
        final_eoc_L2 = log(errors_L2[end-1] / errors_L2[end]) / log(h_list[end-1] / h_list[end])
        if final_eoc_L2 >= 1.0
            println("✓ PASS: Achieved acceptable convergence rate (EOC ≥ 1.0)")
            if final_eoc_L2 >= polyDeg
                println("  Excellent! Even achieved optimal rate despite discontinuous BC")
            end
        else
