...
mutable struct RadialConvDispOp
	...
	function RadialConvDispOp(polyDeg, nCells, col_inner_radius, col_outer_radius, d_rad)
		...
		new(polyDeg, nCells, nNodes, nPoints, strideNode, strideCell, nodes, weights, invWeights, invMM, MM01, MM00, rMM, invrMM, polyDerM, S_g, d_rad_i, deltarho, rho_i, rho_nodes, mul1, c_star, g_star, Dc, Dg, g)
	end
end
...
function parse_variable_coefficient(coeff, ref_radius::Float64)
	if isa(coeff, OrderedDict) || isa(coeff, Dict)
		if haskey(coeff, "type")
			...
			elseif coeff_type == "reciprocal_power"
				# p(rho) = base_val * (rin / rho)^exponent
				# Models D_rad ∝ 1/ρ (exponent=1) or k_f ∝ 1/ρ^(1/3) (exponent=1/3)
				base_val = coeff["base"]
				rin = haskey(coeff, "rin") ? coeff["rin"] : ref_radius
				exponent = haskey(coeff, "exponent") ? coeff["exponent"] : 1.0
				return rho -> base_val * (rin / rho)^exponent
			else
				error("Unknown variable coefficient type: $coeff_type. Supported: linear, reciprocal_power")
			end
		end
	end
...
# Fields
- `nComp`, `col_radius_inner`, `col_radius_outer`, `col_dispersion`, `eps_c`, `Fc`: Physical and geometric parameters.
- `c0`, `cp0`, `q0`: Initial condition vectors.
- `cIn`, `bind`: Inlet concentration vector and binding model.
- `polyDeg`, `nCells`, `ConvDispOpInstance`, `Fjac`, `idx`: DG stuff.
- `bindStride`, `adsStride`, `unitStride`: Strides and indexing parameters.
- `cpp`, `RHS_q`, `qq`, `RHS`, `solution_outlet`, `solution_times`: State and allocation variables.

# Constructor
- `rLRM(; nComp, col_radius_inner, col_radius_outer, col_dispersion, eps_c, c0, cp0, q0, polyDeg, nCells)`
"""
mutable struct rLRM <: ModelBase
	# Check parameters
	# These parameters are the minimum to be specified for the LRM
	nComp::Int64
    col_radius_inner::Float64
    col_radius_outer::Float64
    col_dispersion::Union{Float64, Vector{Float64}, Vector{Function}}
    eps_c::Float64
	c0::Union{Float64, Vector{Float64}} # defaults to 0
	cp0::Union{Float64, Vector{Float64}} # if not specified, defaults to c0
	q0::Union{Float64, Vector{Float64}} # defaults to 0
	...
	# Default variables go in the arguments in the LRM
	function rLRM(; nComp, col_radius_inner, col_radius_outer, col_dispersion, eps_c, c0 = 0.0, cp0 = -1, q0 = 0, polyDeg=4, nCells=8)

		# Normalize col_dispersion first (before creating ConvDispOpInstance)
		if typeof(col_dispersion) == Float64
			col_dispersion_init = col_dispersion  # Use scalar for S_g initialization
			col_dispersion = ones(Float64,nComp) * col_dispersion
		elseif isa(col_dispersion, Function)
			col_dispersion_init = col_dispersion  # Use function for S_g initialization
			col_dispersion = Vector{Function}([col_dispersion for _ in 1:nComp])
		else
			col_dispersion_init = col_dispersion[1]  # Use first component for S_g initialization
		end

		# Get necessary variables for convection dispersion DG
		ConvDispOpInstance = RadialConvDispOp(polyDeg, nCells, col_radius_inner, col_radius_outer, col_dispersion_init)
		...
		# The new commando must match the order of the elements in the struct!
		new(nComp, col_radius_inner, col_radius_outer, col_dispersion, eps_c, c0, cp0, q0, cIn, polyDeg, nCells, ConvDispOpInstance, bindStride, adsStride, unitStride, idx, Fc, Fjac, cpp, RHS_q, qq, RHS, solution_outlet, solution_times, bind)
	end
end
...
- `c0`, `cp0`, `q0`: Initial condition vectors.
- `cIn`, `bind`: Inlet concentration vector and binding model.
- `polyDeg`, `nCells`, `ConvDispOpInstance`, `FilmDiffOpInstance`, `Fjac`, `idx`: DG stuff.
- `bindStride`, `adsStride`, `unitStride`: Strides and indexing parameters.
- `cpp`, `RHS_q`, `qq`, `RHS`, `solution_outlet`, `solution_times`: State and allocation variables.

# Constructor
- `rLRMP(; nComp, col_radius_inner, col_radius_outer, col_dispersion, eps_c, eps_p, kf, Rp, c0, cp0, q0, polyDeg, nCells)`
"""
mutable struct rLRMP <: ModelBase
	...
	function rLRMP(; nComp, col_radius_inner, col_radius_outer, col_dispersion, eps_c, eps_p, kf, Rp, c0 = 0.0, cp0 = -1, q0 = 0, polyDeg=4, nCells=8)
		...
		# The new commando must match the order of the elements in the struct!
		new(nComp, col_radius_inner, col_radius_outer, col_dispersion, eps_c, eps_p, kf, Rp, c0, cp0, q0, cIn, polyDeg, nCells, ConvDispOpInstance, FilmDiffOpInstance, bindStride, adsStride, unitStride, idx, idx_p, Fc, Fp, Fjac, cpp, RHS_q, qq, RHS, solution_outlet, solution_times, bind)
	end
end
...
# Fields
- `nComp`, `col_radius_inner`, `col_radius_outer`, `col_dispersion`, `eps_c`, `eps_p`: Physical and geometric parameters.
...
# Constructor
- `rGRM(; nComp, col_radius_inner, col_radius_outer, col_dispersion, eps_c, eps_p, kf, Rp, Dp, Rc, c0, cp0, q0, polyDeg, polyDegPore, nCells)`
"""
mutable struct rGRM <: ModelBase
	# Physical parameters (combining GRM pore discretization + rLRMP radial geometry)
	nComp::Int64
	col_radius_inner::Float64                                   # Inner radius
	col_radius_outer::Float64                                   # Outer radius
	col_dispersion::Union{Float64, Vector{Float64}, Vector{Function}}  # Radial dispersion
	...
	# Constructor
	function rGRM(; nComp, col_radius_inner, col_radius_outer, col_dispersion, eps_c, eps_p, kf, Rp, Dp, Rc=0.0, c0=0.0, cp0=-1, q0=0, polyDeg=4, polyDegPore=4, nCells=8)

		# 1. Normalize col_dispersion for function/scalar support (from rLRMP pattern)
		if typeof(col_dispersion) == Float64
			col_dispersion_init = col_dispersion
			col_dispersion = ones(Float64, nComp) * col_dispersion
		elseif isa(col_dispersion, Function)
			col_dispersion_init = col_dispersion
			col_dispersion = Vector{Function}([col_dispersion for _ in 1:nComp])
		else
			col_dispersion_init = col_dispersion[1]
		end
		...
		new(nComp, col_radius_inner, col_radius_outer, col_dispersion,
