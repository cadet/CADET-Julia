...
module DGElements
    ...
    function orthonFactor(_polyDeg, a = 0.0, b = 0.0)
        # a = alpha, b = beta
        n = _polyDeg
    ...
    function standard_stiff_Matrix(_nodes,_polyDeg,alpha=0.0,beta=0.0)
        #Derivative Matrix * Mass Matrix (D * M^(0,0))
        return Transpose(derivativeMatrix(_polyDeg,_nodes)) * MMatrix(_nodes,_polyDeg, alpha, beta)
    end
