module RadialConvDispOperatorDG
    ...
    @inline function radialresidualImpl!(Dc, y, idx, _strideNode, _strideCell, _nNodes, _nCells, _deltarho, _polyDeg, _polyDerM, _invMM, _MM01, _MM00, _rMM, _invrMM, S_g, _nodes, _weights, v, d_rad_i, rho_i, cIn, c_star, g_star, Dg, g, mul1)
        ...
        volumeIntegral!(Dc, y, idx, _strideNode, _strideCell, g, _nCells, _nNodes, _polyDerM, _MM00, _invrMM, S_g, _deltarho, v, rho_i, mul1)
    ...
    # Dc += (2/Δρ) * M_ρ^{-1} * (D^T * M^{(0,0)} * v * c - S_g * g)
    @inline function volumeIntegral!(Dc, y, idx, _strideNode::Int64, _strideCell::Int64, g, _nCells::Int, _nNodes::Int, _polyDerM::Matrix{Float64}, _MM00::Matrix{Float64}, _invrMM::Vector{Matrix{Float64}}, S_g::Vector{Matrix{Float64}}, _deltarho::Float64, v, rho_i, mul1::Vector{Float64})
        base = first(idx)
        @inbounds for Cell in 1:_nCells
            cell_idx = (Cell - 1) * _nNodes + 1 : Cell * _nNodes
            @views Dc[cell_idx] .+= (2.0 / _deltarho) .* (_invrMM[Cell] * (v .* transpose(_polyDerM) * (_MM00 * y[base + cell_idx[1] - 1 : base + cell_idx[end] - 1]) .- S_g[Cell] * g[cell_idx]))
        end
        return nothing
    end
    ...
    @inline function computeNumericalFluxes!(c_star, g_star, y, idx, g, _strideNode, _strideCell, _nNodes, _nCells, v, d_rad_i::Vector{Float64}, rho_i::Vector{Float64}, cIn)
        ...
        if v >= 0.0
            # Outward flow: inlet at inner radius (Cell 1), outflow at outer radius (Cell N+1)
            c_star[1] = cIn
            g_star[1] = 0.0
            c_star[_nCells + 1] = y[idx[1] + _nCells * _strideCell - _strideNode]
            g_star[_nCells + 1] = 0.0
        else
            # Inward flow: inlet at outer radius (Cell N+1), outflow at inner radius (Cell 1)
            c_star[1] = y[idx[1]]
            g_star[1] = 0.0
            c_star[_nCells + 1] = cIn
            g_star[_nCells + 1] = 0.0
        end
        return nothing
    end
