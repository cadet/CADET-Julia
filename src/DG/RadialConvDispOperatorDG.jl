module RadialConvDispOperatorDG
    ...
    @inline function radialresidualImpl!(Dc, y, idx, _strideNode, _strideCell, _nNodes, _nCells, _deltarho, _polyDeg, _polyDerM, _invMM, _MM01, _MM00, _rMM, _invrMM, S_g, _nodes, _weights, v, d_rad, rho_i, cIn, c_star, g_star, Dg, g, mul1, mul2)
        ...
        computeNumericalFluxes!(c_star, g_star, y, idx, g, _strideNode, _strideCell, _nNodes, _nCells, v, d_rad, cIn)

        # Dc -= (2/Δρ) * M_ρ^{-1} * (B * v * c* - B_g * g*)
        surfaceIntegral!(Dc, c_star, g_star, _nCells, _nNodes, _invrMM, v, d_rad, rho_i, _deltarho)
    ...
    @inline function surfaceIntegral!(Dc, c_star::Vector{Float64}, g_star::Vector{Float64}, _nCells::Int, _nNodes::Int, _invrMM::Vector{Matrix{Float64}}, v::Float64, d_rad, rho_i::Vector{Float64}, _deltarho::Float64)
        for Cell in 1:_nCells
            @inbounds @simd for Node in 1:_nNodes
                Dc[(Cell - 1) * _nNodes + Node] -= (2.0 / _deltarho) * (_invrMM[Cell][Node, 1] * (-v * c_star[Cell] + rho_i[Cell] * d_rad * g_star[Cell]) + _invrMM[Cell][Node, _nNodes] * (v * c_star[Cell + 1] - rho_i[Cell + 1] * d_rad * g_star[Cell + 1]))
            end
        end
        return nothing
    end
    ...
    @inline function computeNumericalFluxes!(c_star, g_star, y, idx, g, _strideNode, _strideCell, _nNodes, _nCells, v, d_rad, cIn)
        @inbounds for Cell in 2:_nCells
