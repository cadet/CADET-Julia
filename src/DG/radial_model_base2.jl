mutable struct RadialConvDispOp
	...
	function RadialConvDispOp(polyDeg, nCells, col_inner_radius, col_outer_radius; d_rad_const::Union{Nothing,Float64}=nothing, kf_const::Union{Nothing,Float64}=nothing, advection_flux::Symbol=:central)
		...
		new(polyDeg, nCells, nNodes, nPoints, strideNode, strideCell, nodes, invWeights, invMM, polyDerM, invMrhoM, SgMatrix, MKMatrix, deltarho, rho_i, rho_ip1, advection_flux, mul1, c_star, g_star, Dc, Dg, h)
	end
end
...
################################# LUMPED RATE MODEL (rLRM) #################################
	...
	function rLRM(; nComp, col_inner_radius, col_outer_radius, col_height, v, d_rad, eps_c, c0 = 0.0, cp0 = -1, q0 = 0, polyDeg=4, nCells=8, exact_integration=1, advection_flux::Symbol=:central)
		
		# Get necessary variables for convection dispersion DG 
		RadialConvDispOpInstance = RadialConvDispOp(polyDeg, nCells, col_inner_radius, col_outer_radius, d_rad_const = (d_rad isa Float64 ? d_rad : nothing), advection_flux=advection_flux)
		...
		new(nComp, col_inner_radius, col_outer_radius, col_height, v, d_rad, eps_c, c0, cp0, q0, cIn, exactInt, polyDeg, nCells, RadialConvDispOpInstance, bindStride, adsStride, unitStride, idx, Fc, Fjac, cpp, RHS_q, qq, RHS, solution_outlet, solution_times, bind)
	end
end
...
function compute_transport!(RHS, RHS_q, cpp, x, m::rLRM, t, section, sink, switches, idx_units) 
	...
	RadialConvDispOperatorDG.radialresidualImpl!(
		m.RadialConvDispOpInstance.Dg,                       # OUT residual
		cpp, m.idx,                                          # state + slice
		...
		m.RadialConvDispOpInstance.advection_flux,
		r -> switches.ConnectionInstance.u_tot[switches.switchSetup[section], sink] / (2π * r),  # v
		m.d_rad[j],                                          # d_rad (scalar or function)
		m.cIn[j],                                            # Dirichlet inlet
		m.RadialConvDispOpInstance.c_star,
		m.RadialConvDispOpInstance.g_star,
		m.RadialConvDispOpInstance.Dc,                       # aux g buffer
		m.RadialConvDispOpInstance.h,                        # workspace
		m.RadialConvDispOpInstance.mul1,                     # workspace
		m.exactInt,
		m.eps_c,                                             # film params (neutralize by Rp=Inf)
		Inf,
		0.0,                                                 # kf dummy (no film term)
		cpp,                                                 # cp placeholder (shape-compatible)
		:do_nothing, 0.0,
		...
		# Mobile phase RHS 
		@. @views RHS[m.idx .+ idx_units[sink]] = m.RadialConvDispOpInstance.Dg - m.Fc * RHS_q[m.idx]
	end
...
mutable struct rLRMP <: RadialModelBase
	...
	# Default variables go in the arguments in the LRM(..)
	function rLRMP(; nComp, col_inner_radius, col_outer_radius, col_height, v, d_rad, eps_c, eps_p, kf, Rp, c0 = 0.0, cp0 = -1, q0 = 0, polyDeg=4, nCells=8, exact_integration=1, advection_flux::Symbol=:central)
		
		# Get necessary variables for convection dispersion DG 
		RadialConvDispOpInstance = RadialConvDispOp(polyDeg, nCells, col_inner_radius, col_outer_radius, d_rad_const = (d_rad isa Float64 ? d_rad : nothing), kf_const   = (kf   isa Float64 ? kf   : nothing), advection_flux=advection_flux)
		...
		# The new commando must match the order of the elements in the struct!
		new(nComp, col_inner_radius, col_outer_radius, col_height, v, d_rad, eps_c, eps_p, kf, Rp, c0, cp0, q0, cIn, exactInt, polyDeg, nCells, RadialConvDispOpInstance, bindStride, adsStride, unitStride, idx, idx_p, Fc, Fp, Fjac, cpp, RHS_q, qq, RHS, solution_outlet, solution_times, bind)
	end
end
...
function compute_transport!(RHS, RHS_q, cpp, x, m::rLRMP, t, section, sink, switches, idx_units)
	...
	RadialConvDispOperatorDG.radialresidualImpl!(
		...
		m.RadialConvDispOpInstance.MKMatrix,
		m.RadialConvDispOpInstance.advection_flux,
		r -> switches.ConnectionInstance.u_tot[switches.switchSetup[section], sink] / (2π * r),
		m.d_rad[j],
		m.cIn[j],
		m.RadialConvDispOpInstance.c_star,
		m.RadialConvDispOpInstance.g_star,
		...
		m.exactInt,
		1.0,                                                # eps_c=1 ⇒ film prefactor zero
		Inf,                                                # Rp=Inf ⇒ film prefactor zero
		m.kf[j],                                            # kf (can be function or scalar)
		cpp,                                                # any placeholder; term is zeroed
		:do_nothing, 0.0,
		m.RadialConvDispOpInstance.rho_i,
		m.RadialConvDispOpInstance.rho_ip1,
		m.col_inner_radius # rho_inner (column inner radius)
	)

		# Mobile phase
		@. @views RHS[m.idx .+ idx_units[sink]] = m.RadialConvDispOpInstance.Dg - m.Fc * 3 / m.Rp * m.kf[j] * (x[m.idx .+ idx_units[sink]] - x[m.idx_p .+ idx_units[sink]])
...
mutable struct rGRM <: RadialModelBase
	...
	# Default variables go in the arguments in the LRM(..)
	function rGRM(; nComp, col_inner_radius, col_outer_radius, col_height, v, d_rad, eps_c, eps_p, kf, Rp, Dp, Rc=0.0, c0 = 0.0, cp0 = -1, q0 = 0, polyDeg=4, polyDegPore=4, nCells=8, exact_integration=1, advection_flux::Symbol=:central)
		...
		# The new commando must match the order of the elements in the struct!
		new(nComp, col_inner_radius, col_outer_radius, col_height, v, d_rad, eps_c, eps_p, kf, Rp, Rc, Dp, c0, cp0, q0, cIn, exactInt, polyDeg, nCells, polyDegPore, RadialConvDispOpInstance, PoreOpInstance, bindStride, adsStride, unitStride, idx, idx_p, idx_q, Fc, Fp, Fjac, Jr, invRi, cpp, RHS_q, qq, solution_outlet, solution_times, bind)
	end
end
...
function compute_transport!(RHS, RHS_q, cpp, x, m::rGRM, t, section, sink, switches, idx_units)
	...
	RadialConvDispOperatorDG.radialresidualImpl!(
		...
		# Mobile phase
		@. @views RHS[m.idx] = m.RadialConvDispOpInstance.Dg - m.Fc * 3 / m.Rp * m.kf[j] * (x[m.idx] - x[m.idx_p])
