...
def plot_convergence(CADETFVdata,CADETJuliadata,CADETDGdata=[],profileData=[],saveLocation="",type="LRM"):
    ...
    plt.savefig(os.path.join(saveLocation,'Plot_linear_convergence.svg'),format = 'svg',dpi = 1200, bbox_inches='tight')
    ...
    #     for i in range(CADETDGdata[tag].nunique()):
    #         idx = slice(i * CADETDGdata["nCellu"].nunique(), (i + 1) * CADETDGdata["nCellu"].nunique())
    #         ax.loglog(CADETDGdata['runtime_e_ode'][idx],CADETDGdata["maxE_e_ode"][idx],'.--', label = f'CADET-DG, Exact-ode, $N_p$={CADETJuliadata[tag][idx].min()}' )
            # ax.loglog(CADETDGdata['runtime_i_ode'][idx],CADETDGdata["maxE_i_ode"][idx],'.--', label = f'CADET-DG, Collocation-ide, $N_p$={CADETJuliadata[tag][idx].min()}' )
    ...
    if 'SMA' in saveLocation: #if having the SMA isotherm
        ...
        for i in range(x_columns - 2):
            idxx = "x{}".format(i + 2)
            ax.plot( profileData["x{}".format(x_columns)],profileData[idxx], label="c{} - #Cells = {}".format(i + 1, profileData["nCell1"][0]))
            idxx = "y{}".format(i + 2)
            ax.plot(profileData["y{}".format(x_columns)],profileData[idxx], label="c{} - #Cells = {}".format(i + 1, profileData["nCell2"][0]))
    ...
    if 'SMA' in saveLocation: #if having the SMA isotherm
        ...
        for i in range(x_columns - 2):
            idxx = "x{}".format(i + 2)
            ax[0].plot( profileData["x{}".format(x_columns)],profileData[idxx], label="c{} - #Cells = {}".format(i + 1, profileData["nCell1"][0]))
            idxx = "y{}".format(i + 2)
            ax[0].plot(profileData["y{}".format(x_columns)],profileData[idxx], label="c{} - #Cells = {}".format(i + 1, profileData["nCell2"][0]))
    else: #If not having the SMA isotherm
        #Counting number of components as number of x-1
        x_columns = len(set([col for col in profileData.columns if col.startswith('x')]))
        for i in range(x_columns - 1):
...
def plot_initiator(path)
    ...
    profileData = pd.read_csv(path + 'Profiles_data.csv')
    if path[-2] == "G" # If using the GRM
