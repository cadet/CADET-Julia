...
c_analytical = CSV.read((joinpath(@__DIR__,"Semi-analytical_LRM_SMA.csv")),DataFrame)

# For this particular problem, we need to use the SUNDIALS CVODE_BDF() solver 
using Sundials

function model_setup(nCells, polyDeg, exactInt, analJac=false, solver=CVODE_BDF())

	# Specify transport parameters for LRMP
	model = LRM(
		nComp = 4, 
		colLength = 0.014, 
		d_ax = [1e-4,1e-4,1e-4,1e-4], 
		eps_c = 0.8425, 
		u = 5.75e-4, 
