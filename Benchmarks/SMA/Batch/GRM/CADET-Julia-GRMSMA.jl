...
c_analytical = CSV.read((joinpath(@__DIR__,"Semi-analytical_GRM_SMA.csv")),DataFrame)

function model_setup(nCells, polyDeg, polyDegPore, exactInt)

	# Specify transport parameters for LRMP
	model = GRM(
		nComp = 4, 
		colLength = 0.014, 
		d_ax = [5.75e-8, 5.75e-8, 5.75e-8, 5.75e-8], 
		eps_c = 0.37, 
		eps_p = 0.75,
		Rp = 4.5e-5, 
		u = 5.75e-4, 
		kf = [6.9e-6, 6.9e-6, 6.9e-6, 6.9e-6 ],
		Dp = [70e-11, 6.07e-11, 6.07e-11, 6.07e-11],
